...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    data = { title: 'Fam', body: event.data ? event.data.text() : '' }
  }
  const title = data.title || 'Fam'
  const options = {
    body: data.body || '',
    icon: '/icon-192.png',
    badge: '/icon-192.png',
    data: { url: data.url || '/vandaag' },
  }
...
self.addEventListener('notificationclick', (event) => {
  ...
  const url = (event.notification.data && event.notification.data.url) || '/vandaag'
  event.waitUntil(
